...
export default function PaymentsPage() {
  ...
  useEffect(() => {
	...
	const fetchPaymentDockey = async () => {
		try {
			const response = await api.get<Payment>(`/payments/${dockey}`);
			...
			alert("Failed to fetch payment. Please try again.");
			router.push('/module/payments');
			}
		} finally {
			setLoading(false);
		}
  ...
  return (
	<div className="space-y-6">
		...
		<Breadcrumb
			items={[
				{ label: 'Payment', href: '/module/payments' },
				{ label: payment?.docno ?? 'View Payment' }
			]}
		/>
		<PaymentForm initialData={payment} mode={mode} />
	</div>
  );
}