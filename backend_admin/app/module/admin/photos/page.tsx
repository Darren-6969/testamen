...
export default function PhotosTab() {
  ...
  // files picked but not yet uploaded -- the description dialog gates them.
  // albumId set means they were picked from inside an open album.
  ...
  return (
    <div className="space-y-5">
      <input ref={bgInput} type="file" accept="image/*" multiple hidden onChange={(e) => { onUploadBackgrounds(e.target.files); e.target.value = ''; }} />
      <input ref={photoInput} type="file" accept="image/*" multiple hidden onChange={(e) => { stagePhotos(e.target.files); e.target.value = ''; }} />
      <input ref={albumUploadInput} type="file" accept="image/*" multiple hidden onChange={(e) => { stagePhotos(e.target.files, openAlbum?.id); e.target.value = ''; }} />
