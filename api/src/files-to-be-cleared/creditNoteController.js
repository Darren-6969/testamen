...
exports.getAllCreditNotes = async (req, res) => {
  try {
    ...
    if (cursor?.docdate && cursor?.dockey != null) {
      ...
      cursorClause = `AND (cn.docdate < $${p1} OR (cn.docdate = $${p1} AND cn.dockey < $${p2}))`;
    }
    ...
    const rows = await runQuery(db, `
      SELECT
        ...
        c.companyname,
        cb.attention
      FROM billing_fb.ar_cn cn
      LEFT JOIN billing_fb.ar_customer c ON c.code = cn.code
      LEFT JOIN (
        SELECT DISTINCT ON (code) code, attention
        FROM billing_fb.ar_customerbranch
        ORDER BY code, dtlkey
      ) cb ON cb.code = cn.code
      WHERE cn.cancelled IS NOT TRUE
        AND ($2 = '' OR cn.docno ILIKE '%' || $2 || '%')
        AND ($3 = '' OR c.companyname ILIKE '%' || $3 || '%')
        AND ($4 = '' OR cb.attention ILIKE '%' || $4 || '%')
        AND ($5 = '' OR cn.docdate::text ILIKE '%' || $5 || '%')
        AND ($6 = '' OR cn.description ILIKE '%' || $6 || '%')
        AND ($7 = '' OR cn.localdocamt::text ILIKE '%' || $7 || '%')
        ${cursorClause}
      ORDER BY cn.docdate DESC, cn.dockey DESC
      LIMIT $1
    `, params);
