...
exports.getAllUsers = async (req, res, next) => {
  try {
    ...
    const defaultQuery = `SELECT * FROM users LEFT JOIN user_role ON users.role_id = user_role.id`;

    // Query with selected fields if provided
    const queryWithCond = `SELECT ${fieldList} FROM users LEFT JOIN user_role ON users.role_id = user_role.id`;
...
exports.checkUsernameExists = async (req, res) => {
  try {
    ...
    const result = await runQuery(db, query, [username]);
