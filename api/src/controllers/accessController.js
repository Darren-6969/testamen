...
// ============================================================================
exports.getUserModuleAccess = async (req, res, next) => {
  try {
    ...
    const defaultQuery = `SELECT user_module_access.* FROM user_module_access LEFT JOIN users ON user_module_access.group_id = users.role_id WHERE users.id = ${req.user.userId} ORDER by module_id`;
    // const defaultQuery = `SELECT user_module_access.* FROM user_module_access LEFT JOIN users ON user_module_access.user_id = users.id WHERE user_module_access.user_id = ${req.user.userId} ORDER by module_id`;

    // Query with selected fields if provided
    const queryWithCond = `SELECT ${fieldList} FROM user_module_access LEFT JOIN users ON user_module_access.group_id = users.role_id WHERE users.id = ${req.user.userId} ORDER by module_id`;
    // const queryWithCond = `SELECT ${fieldList} FROM user_module_access LEFT JOIN users ON user_module_access.user_id = users.id WHERE user_module_access.user_id = ${req.user.userId} ORDER by module_id`;
...
exports.getUserSubmoduleAccess = async (req, res, next) => {
  try {
    // Extract requested fields
    const { fields } = req.body || {};
    let fieldList;
    ...
    // Default query (all columns + join with user_role)
    const defaultQuery = `SELECT * FROM user_submodule_access LEFT JOIN users ON user_submodule_access.group_id = users.role_id WHERE users.id = '${req.body.userId}' ORDER by module_id`;
    // const defaultQuery = `SELECT * FROM user_submodule_access LEFT JOIN users ON user_submodule_access.user_id = users.id WHERE user_submodule_access.user_id = '${req.body.userId}' ORDER by module_id`;

    // Query with selected fields if provided
    const queryWithCond = `SELECT ${fieldList} FROM user_submodule_access LEFT JOIN users ON user_submodule_access.group_id = users.role_id WHERE users.id = '${req.body.userId}' ORDER by module_id`;
    // const queryWithCond = `SELECT ${fieldList} FROM user_submodule_access LEFT JOIN users ON user_submodule_access.user_id = users.id WHERE user_submodule_access.user_id = '${req.body.userId}' ORDER by module_id`;
...
// ]
// ============================================================================
exports.getFullModuleList = async (req, res, next) => {
  try {
